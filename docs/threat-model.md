...
- visor-gateway requires RFC 9449 DPoP bound to minted `cnf.jkt` (a workload-possessed key), including a server-issued nonce. STS exchange still uses `actor_token`, with optional token-endpoint DPoP to bind `cnf.jkt`. This is not a Production identity plane.
